...
lmmfit <- function(Y, X, Z, d, sigma2 = NULL, method = "REML-FS", max.iter = 50, epsilon = 1e-5, output.cov = TRUE)
{
...
##LMM fitting
lmm(XY, ZX, ZY, ZZ, XXinv = XXinv, Ynorm = Ynorm, n = n, d = d, 
	sigma2 = sigma2, method = method, max.iter = max.iter, epsilon = epsilon, output.cov = output.cov)
}
